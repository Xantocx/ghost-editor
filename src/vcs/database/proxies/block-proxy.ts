...
import { FileProxy, LineProxy, TagProxy, VersionProxy } from "../proxy-types"
import { VCSBlockId, VCSBlockInfo, VCSBlockRange, VCSFileId, VCSTagInfo } from "../../provider"
...
import { ISessionBlock } from "../session"
import TimestampProvider from "../utils/timestamp-provider"

export default class BlockProxy extends DatabaseProxy implements ISessionBlock<FileProxy, BlockProxy, LineProxy, TagProxy> {
    ...
    public static async loadFrom(block: Block): Promise<BlockProxy> {
        const proxy = new BlockProxy(block.id, block.blockId, block.type as BlockType, block.timestamp)
    ...
    public async applyIndex(targetIndex: number): Promise<void> {
        ...
        const selectedVersion = timeline[targetIndex] // actually targeted version
    ...
    public async updateLines(fileId: VCSFileId, change: MultiLineChange): Promise<VCSBlockId[]> {
        ...
        */
        ...
        let   latestTimestamp:  number               = this.timestamp

        const deleteLine = async (line: LineProxy) => {
        ...
        const updateLine = async(line: LineProxy, content: string) => {
            affectedLines.push(line)
            latestTimestamp = TimestampProvider.getTimestamp()
            const responsibleBlock = table.get(line.id)
    ...
    public async createTag(options?: { name?: string, description?: string, codeForAi?: string }): Promise<TagProxy> {
        ...
        const codeForAi   = options?.codeForAi ? options.codeForAi : code
