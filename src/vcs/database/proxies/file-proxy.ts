...
import { LineProxy, VersionProxy, BlockProxy } from "../proxy-types"
import { prismaClient } from "../client"
import TimestampProvider from "../utils/timestamp-provider"
import { ProxyCache } from "../utils/cache"
import { ISessionFile } from "../session"

export default class FileProxy extends DatabaseProxy implements ISessionFile {
    ...
    public static async create(filePath: string | undefined, eol: string, content: string | undefined): Promise<{ file: { filePath: string, file: FileProxy }, rootBlock: { blockId: string, block: BlockProxy } }> {
        ...
        const file = await prismaClient.file.create({
            data: {
                ...
                eol,
                lines: {
                    create: lineContents.map((_, index) => {
                        return { 
                            order: index + 1,
                    ...
                    createMany: {
                        data: lineContents.map((_, index) => {
                            return { 
                                order: index + 1,
                                type: LineType.ORIGINAL as string,
                            }
                        })
                    }
                    */
        ...
        await prismaClient.$transaction(lineContents.map((content, index) => {
            return prismaClient.version.create({
                data: {
                    lineId: file.lines[index].id,
    ...
    public async insertLines(lineContents: string[], relations?: { previous?: LineProxy, next?: LineProxy, sourceBlock?: BlockProxy }): Promise<{ line: LineProxy, v0: VersionProxy, v1: VersionProxy }[]> {
        ...
        const versionData: Prisma.VersionUncheckedCreateWithoutLineInput[][] = lineContents.map(content => {
            return [
        ...
        const newLines = lineData.map(({ line }) => line)
