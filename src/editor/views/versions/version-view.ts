...
export abstract class VersionViewContainer<Version, CustomView extends VersionViewElement<Version, CustomView, VersionViewContainer<Version, CustomView>>> extends View implements IVersionViewContainer<Version> {
    ...
    protected abstract createCustomView(version: Version): Promise<CustomView>
    ...
    public onVersionsChange(callback: (versions: Version[]) => void): Disposable {
        ...
        return this.addSubscription({
            dispose: () => {
                const index = this.versionsChangedCallbacks.indexOf(callback, 0)
                if (index > -1) { this.versionsChangedCallbacks.splice(index, 1) }
            },
        })
    }
...
export abstract class TagViewContainer<CustomView extends TagViewElement<CustomView, TagViewContainer<CustomView>>> extends VersionViewContainer<VCSVersion, CustomView> {}
export type ITagViewContainer = IVersionViewContainer<VCSVersion>