...
class ReactViewWrapper extends ViewWrapper<View, void> {
    ...
    private readonly reactNode: ReactNode

    private currentRoot?: Root

    public constructor(root: HTMLElement, reactNode: ReactNode, show?: boolean) {
        super(root)
        this.reactNode = reactNode
        if (show)    { this.show() }
    }

    public override async show(): Promise<View | undefined> {
        if (!this.isVisible) {
            this.currentRoot?.unmount()
            this.currentRoot = createRoot(this.container)
            this.currentRoot.render(this.reactNode)
        }
...
export default class MetaView extends View {
    ...
    public async addReactView(identifier: ViewIdentifier, reactNode: ReactNode, show?: boolean): Promise<void> {
        if (this.views.has(identifier)) { throw new Error(`Please remove the existing view for ${identifier} before adding a new one!`) }

        const wrapper = new ReactViewWrapper(this.root, reactNode)
        this.setIdentifier(identifier, wrapper)
